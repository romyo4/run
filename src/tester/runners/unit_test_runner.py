"""Unit Testの実行(IS10 4.3節 run_unit_tests)。
...
(判定は`tester.quality_gate.judge_unit_test`に集約)。
"""
...
def run_unit_tests(
    ...
    """設定されたUnit Testコマンドを実行し、結果を`TestExecutionReport`として返す。"""
    try:
    ...
    command = list(config.unit_test_command)
    try:
