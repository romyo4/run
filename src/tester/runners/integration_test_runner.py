"""Integration Testの実行(IS10 4.3節 run_integration_tests)。
...
(判定は`tester.quality_gate.judge_integration_test`に集約)。
"""
...
def run_integration_tests(
    ...
    """設定されたIntegration Testコマンドを実行し、結果を`TestExecutionReport`として返す。"""
    try:
    ...
    command = list(config.integration_test_command)
    try:
